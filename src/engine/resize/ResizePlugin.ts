...
export class CreationResizePlugin {
  ...
  private static _queueResizeListener: (() => void) | null;
  ...
  public static init(options: ApplicationOptions): void {
    ...
    Object.defineProperty(
       ...
       */
      {
        set(dom: Window | HTMLElement) {
          if (this._queueResizeListener) {
            globalThis.removeEventListener('resize', this._queueResizeListener);
          }
          this._resizeTo = dom;
          if (dom && this._queueResizeListener) {
            globalThis.addEventListener('resize', this._queueResizeListener);
            app.resize();
          }
        },
        get() {
     ...
     * only be called once.
     */
    this._queueResizeListener = (): void => {
      if (!this._resizeTo) {
        return;
      }
    ...
    app.queueResize = this._queueResizeListener;
  ...
  public static destroy(): void {
    ...
    this._queueResizeListener = null;
