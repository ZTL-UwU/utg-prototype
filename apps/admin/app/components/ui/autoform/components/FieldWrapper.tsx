  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {!hideHelperText && parsedField.fieldConfig?.description && (
        <div className="text-sm text-muted-foreground">{parsedField.fieldConfig.description}</div>
      )}
      {!hideHelperText && error && <div className="text-sm text-destructive">{error}</div>}
    </div>
  );
};
