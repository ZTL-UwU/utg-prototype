...
/** Book fills most of the viewport; bottom may extend past the screen edge. */
...
export class TypingSentenceScreen extends Container {
  public static assetBundles = ['typing-level-6', REMOTE_SENTENCES_BUNDLE];
  public static splashBackgroundAsset = 'typing-levels/typing-level-6/background.png';
  ...
  constructor(level: TLevel) {
    const typedLevel = getTypedLevel(level, 'typing-story');
    ...
    this.background = new Sprite({
      texture: Texture.from('typing-levels/typing-level-6/background.png'),
      layout: { position: 'absolute', width: '100%', height: '100%', objectFit: 'cover' },
    });

    this.book = new Sprite({
      texture: Texture.from('typing-levels/typing-level-6/book.svg'),
      anchor: 0.5,
    });
